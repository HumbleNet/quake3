 ...
 */
int HUMBLENET_API humblenet_select( int nfds, fd_set *readfds, fd_set *writefds,
...
#define IS_HUMBLENET_HOST( h ) ((strlen(h) > humblenet_domain_length) && (0 == strncasecmp(h+strlen(h)-humblenet_domain_length, humblenet_domain, humblenet_domain_length)))
...
int hs_getaddrinfo( const char* node, const char* service, const struct addrinfo* hints, struct addrinfo** res ) {
	...
	if( service == NULL && IS_HUMBLENET_HOST( node ) ) {
        ...
        if( node[0] == 'p' && node[1] == 'e' && node[2] == 'e' && node[3] == 'r' && node[4] == '_' )
			...
			addr.sin_addr.s_addr = htonl( humblenet_p2p_lookup_peer_by_name( node ) );
