...
static qboolean NET_GetCvars( void ) {
    ...
    // asmjs only supports humblenet for a network
    net_enabled = Cvar_Get( "net_enabled", "16", CVAR_LATCH | CVAR_ARCHIVE );
    if( net_enabled->integer && net_enabled->integer != 16 )
        Cvar_Set("net_enabled", "16" );
#else
